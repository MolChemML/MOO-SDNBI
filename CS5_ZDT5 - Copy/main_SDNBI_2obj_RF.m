...
% Load necessary data
load('PF_CS5_oNBI.mat'); % load true Pareto front


tMainStart      = tic;    % Log cpu time (MATLAB simulation)
dim             = 2;      % Number of objectives 
n_dim           = 11;      % number of decision variables
n_const         = 2;      % number of constratins (equality + inequality)
n_sobol         = 30;     % number of starting points (generated by Sobol' sequnce)
dTol            = 0.005;  % Convergence critera for the algorithm (error between inner and outer)
sTol            = 0.05;    % Convergence criteria for the algorithm (spreadness) 
dMeasure        = 0.005;   % To check (spreadness) 
...
%}
 ...
 load('NBI_2obj_cs5_anchor.mat'); 
 PPoints(1,end+1:end+3)=[0 0 0];
...
while dMax > dTol
  ...
  for n=n1:idx(end,1)  % iterations for subspaces)
      ...
      plot(PF_CS5_oNBI(:,dim+1),PF_CS5_oNBI(:,dim+2),'o','MarkerEdgeColor',rgb('LightGray'),'MarkerSize',3);
      ...
      % =================================================================
      x_ini = PPoints(index_sub(2:end),2*dim+1:2*dim+n_dim);
      [objsNew, n_groupsNew,properties, tStore, tElapsed] = ...
      ...
      if any(diff < 1e-5)
        ...
        [objsNew, n_groupsNew, properties_e, tStore_e, tElapsed_e] ...
            = f_MtoG_mNBIn_2obj(weight, beta, normal, phi, f_o, f_k, icut,dim,n_dim, n_sobol, x_ini, flag_b);
      ...
      if index_dontadd == 0
         PPoints(end+1,:) = ...
          ...
          w_correct = PPoints(end,4*dim+n_dim:4*dim+n_dim+1)/sum(PPoints(end,4*dim+n_dim:4*dim+n_dim+1)); % from Lagrangian multiplier
