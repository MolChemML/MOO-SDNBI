...
n_dim           = 11;
n_sobol         = 30;
dTol            = 0;  % Convergence critera for the algorithm
iter            = 1 ;
%% Initialise Array
...
for i = 1:size(wMat,1)
    ...
    [objsNew, n_groupsNew, properties, tStore, tElapsed]  = ...
                 f_MtoG_WS_2obj(wMat(i,:),dim,n_dim,n_sobol);
...
%% 4. Select A Facet with the Largest Error
 ...
 %% 5. Determine New Point Z* By Solving z* argmin{wTz]
    ...
    [objsNew, n_groupsNew,properties,tStore, tElapsed] = ...
        f_MtoG_WS_2obj(w_norm,dim,n_dim,n_sobol);
...
%  ASK TO ED
    ...
    PPointsHistory(end+1,:) = PPoints(end,:);
     ...
     filename = sprintf('CS5_SD_final_Run.mat',iter);
     save(filename);
     iter=iter+1;
...
PPoints_CS5_SD = PPoints;
z_final = PPoints(1:26,3:4);
f_u = [7.2509860, 0.5]; f_n = [18.50000091, 3.04535252];
z_final(:,1)=(f_n(1)-f_u(1))*z_final(:,1)+f_u(1);
z_final(:,2)=(f_n(2)-f_u(2))*z_final(:,2)+f_u(2);
hv_CS5_SD=approximate_hypervolume_ms(PPoints(1:26,3:4)',[1,1]')
PPoints_r = sortrows(PPoints(1:26,:),3,'ascend');
crowding_dist=compute_crowding_distances(PPoints_r(:,3:4)');
CD_CS5_SD=crowding_dist;
CD_stdv_CS5_SD=std(CD_CS5_SD(:,2:end-1)');
CD_avg_CS5_SD =mean(CD_CS5_SD(:,2:end-1)');
DM_CS5_SD = compute_quality_pareto(PPoints(1:26,3:4), [1,1], [0,0]);

save('CS5_SD_210217_total46iter.mat');