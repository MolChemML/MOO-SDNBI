...
function [objs, n_groups, properties, tWall, tCPU] ...
        = f_MtoG_NBI_2obj(weight, beta1, normal1, phi1, fo1, fok, icut1, dim, n_dim, n_sobol)
...
%==========================================================================
    ...
    GAMS.putGDX('inputcs3_nbi.gdx',i,j,m,beta,normal,f_o,f_k,phi,x_i);
...
%==========================================================================
    ...
    g = GAMS(struct('model','NBI_CS3.gms'));
...
%==========================================================================
    ...
    if g.status == 0
        ...
        status1 = copyfile('NBI_CS3_result.txt', 'results2.txt');
        %status2 = copyfile('NBI_CS3_WS.log', 'log2.txt');
        status2 = copyfile('NBI_CS3.log', 'log2.txt');
...
%==========================================================================
       [M, index] = max(properties_i(:,1));
        ...
        if sum(abs(objs - fok(1,:))) < 1e-4 || sum(abs(objs - fok(2,:))) < 1e-4
           objs;
        end
